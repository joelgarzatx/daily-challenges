...
var Allergies = function(score){
	...
	this.maxScore = function() {
		var sum = 0;
		for(allergy in this.allergens) {
		  sum += this.allergens[allergy];
		}
		return sum + 1;
	}
};
...
Allergies.prototype.list = function(){
	localScore = this.score % this.maxScore();
	
	list = [];
	for(allergy in this.allergens) {
		if(localScore >= this.allergens[allergy]) {
			list.unshift(allergy);
			localScore -= this.allergens[allergy];
		}
	}
	return(list);
};
